...
This module's panel layout is anchored, verbatim, to the confirmed
mockups in Sections 8.3.5/8.3.6:
...
The Confirmation + Discussion Loop panel (8.3.3) follows the same visual
language by design, since no separate mockup constrains it:
  ...
  - Risk-level text color: low=`omsh.risk.low`, medium=`omsh.risk.medium`,
    high=`omsh.risk.high`, used only as inline text styling.

Every color reference in this module is one of ui/theme.py's `"omsh.*"`
style names (see that module's docstring for the palette rationale) --
`print_panel`/`RichSudoPrompt` construct their fallback `Console` via
`ui.theme.themed_console()` rather than a bare `rich.console.Console()`
so those style names resolve wherever no console is injected by a caller.
"""
...
def _key_hint_text(line: str) -> Text:
    """
    Styles every "[key] Label" option-hint line in this module: each
    "[key]" token is colored with `omsh.accent` -- this app's own
    signature accent, already used for /help's own command names and the
    palette's command-name column (ui/palette.py) -- while the label text
    after it stays `omsh.muted`, so a person scanning a panel can pick out
    "what key do I press" at a glance.
...
def _telemetry_footer_text(telemetry: ParseTelemetry, *, attempts: int | None = None) -> Text | None:
    ...
    "↯ 94 tokens in · 62 tokens out · 0.8s · gemini-3.5-flash-lite" -- shown
    below a horizontal rule inside the same panel box. Extended with
    tokens/sec and a retry count when relevant -- tokens_in doubles as the
    context/prompt-size figure, since that number already is the size of
    the prompt sent to the model.

    Any field ParseTelemetry doesn't have (a backend that couldn't report
    it) is left out of this line rather than shown as a placeholder like
    "? tokens" -- an honest partial line beats a fake-looking complete
    one. Returns None only when nothing is available at all (every field
    is None and no retry happened), so an empty telemetry object doesn't
    add an empty footer/rule to the panel.

    `attempts` (from ParseResult.attempts) adds "· N retries" only when it
    is 2 or more -- a normal, single-attempt call shows nothing extra.
    """
    parts: list[str] = []
...
class RichSudoPrompt:
    ...
    `input_fn`, when not overridden, defaults to
    ...
    keys, rather than a line-editing read compared against typed text
    after Enter (see `read_plan_choice_keypress`'s own docstring for why
    that distinction matters). `input_fn` stays overridable (main.py's
    edit/chat-adjust sub-prompts, and every existing test in this module,
    pass an explicit `input_fn` lambda); only the plain no-argument
    construction path is the one real callers hit during normal operation.
    """
